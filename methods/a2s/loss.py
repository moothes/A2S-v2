...
def ACLoss(pred, image, radius, config=None): # rgb=200
        rgb = config['rgb']
        modal = config['trset']
        ...
        for idx, slice in enumerate(slices):
            if idx == len(slices) - 1:
                continue
            dis_map = (-rgb * kernels[:, slice:slices[idx+1]] ** 2).sum(dim=1, keepdim=True).exp()
            # Only RGB
...
def Loss(pre1, img1, pre2, img2, epoch, ws, config):
    ...
    adb_loss = ADBloss(p1, pre1['feat'][0], False, epoch, config) + ADBloss(p2, pre2['feat'][0], False, epoch, config)
